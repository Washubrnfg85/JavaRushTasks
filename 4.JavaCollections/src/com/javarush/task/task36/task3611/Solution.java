...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        Set<Integer> allSuitable = new HashSet<>();
        if (deep == 0) {
            return allSuitable;
        } else {
            ...
            Object[] arrayToProcess = allSuitable.toArray();
            for(Object value : arrayToProcess) {
                allSuitable.addAll(getAllFriendsAndPotentialFriends((Integer) value, deep - 1));
            }
            allSuitable.remove(index);
        }
